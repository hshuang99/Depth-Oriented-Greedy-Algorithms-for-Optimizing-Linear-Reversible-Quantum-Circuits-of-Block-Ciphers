...
operations used. If the reduced result is not equal to $M$, this indicates 
that the CNOT gates are valid; otherwise, if the result remains unchanged, 
the operators are invalid, and the corresponding CNOT gates are identified 
as ineffective.
...
\begin{algorithmic}[1]
\Require $M \in \mathbb{F}_{2}^{n \times n}, OPs_{r}, OPs_{c}$
\Ensure Check operations from $OPs_{r}$ and $OPs_{c}$ that can work on $M$
\State reduce $\leftarrow$ $M$ \Comment{Copy $M$ into a variable to be reduced.}
...
\If{reduce $\neq M$} \Comment{Check whether ``reduce`` is equal to $M$.}
